import {renderCalculatorPilot, renderLogPilot} from './care-tools-pilot.js';
import {renderChecklistComplete, renderNameFinderComplete} from './care-organizers.js';
...
for (const root of document.querySelectorAll('[data-care-tool]')) {
  const configNode=root.querySelector('[data-tool-config]'), runtime=root.querySelector('[data-tool-runtime]');
  if(!configNode||!runtime)continue;
  const config=JSON.parse(configNode.textContent),labels=runtimeText[document.documentElement.lang]||runtimeText.en;
  const renderers={calculator:()=>renderCalculatorPilot(config,labels),log:()=>renderLogPilot(config,labels),checklist:()=>renderChecklistComplete(config),nameFinder:()=>renderNameFinderComplete(config)};
  if(renderers[config.type])runtime.replaceChildren(renderers[config.type]());
}
